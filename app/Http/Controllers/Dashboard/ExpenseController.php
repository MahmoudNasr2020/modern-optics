...
class ExpenseController extends Controller
{
    ...
    public function salariesPreview(Request $request)
    {
        $user = auth()->user();

        // Build users query
        $query = \App\User::with('branch')
            ->where('is_active', true)
            ->where('salary', '>', 0); // فقط اللي عندهم راتب محدد

        // Branch filtering - نفس منطق بقية المتودز
        $selectedBranchId = $user->getFilterBranchId($request->branch_id);

        if ($selectedBranchId !== null) {
            // فلتر لفرع معين
            $query->where('branch_id', $selectedBranchId);
        } elseif (!$user->isSuperAdmin() && !$user->can('access-all-branches')) {
            // مستخدم عادي بدون فرع محدد → فرعه فقط
            $query->where('branch_id', $user->branch_id);
        }
        // لو null وهو Super Admin → كل الموظفين
        ...
        $formatted = $employees->map(function ($emp) {
            return [
                ...
                'email'       => $emp->email,
                'salary'      => $emp->salary,
                'branch_name' => optional($emp->branch)->name ?? 'No Branch',
                'branch_id'   => $emp->branch_id,
                'role'        => $emp->roles->pluck('name')->implode(', ') ?: 'N/A',
            ];
        });
