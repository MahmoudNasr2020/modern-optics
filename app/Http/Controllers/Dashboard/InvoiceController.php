...
// في InvoiceController.php
    ...
    public function getPendingInvoices(Request $request)
    {
        $invoices = Invoice::leftJoin('users', 'users.id', 'invoices.user_id')
            ...
            ->where('invoices.status', 'pending')->orderBy('invoices.created_at', 'DESC');

        // Apply filters
        if ($request->invoice_code) {
            $invoices->where('invoices.invoice_code', 'like', '%' . $request->invoice_code . '%');
        }
        if ($request->customer_code) {
            $invoices->where('invoices.customer_id', 'like', '%' . $request->customer_code . '%');
        }
        if ($request->search) {
            $invoices->where('customers.english_name', 'like', '%' . $request->search . '%');
        }
        if ($request->creation_date) {
            $invoices->whereDate('invoices.created_at', $request->creation_date);
        }

        $invoices = $invoices->get();

        // Flag each invoice: does it have lens items? does it have an active PO?
