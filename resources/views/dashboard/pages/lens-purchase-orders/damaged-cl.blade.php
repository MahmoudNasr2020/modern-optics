...
@section('title', 'هالك — Damaged Contact Lenses Management')
@section('content')
...
.entry-row:hover { box-shadow:0 3px 14px rgba(21,101,192,.12); }
.entry-row.fully-recovered { border-left-color:#27ae60; opacity:.65; }

.cl-name  { font-size:14px; font-weight:700; color:#2c3e50; }
.cl-meta  { font-size:12px; color:#888; margin-top:3px; }
...
<section class="content-header">
    <h1>
        <i class="bi bi-exclamation-triangle-fill" style="color:#1565c0;"></i>
        هالك — Damaged Contact Lenses
...
<div class="halak-page">
    ...
    <div class="halak-header">
        <div class="row" style="align-items:center;">
            <div class="col-md-7">
                <h2><i class="bi bi-eye" style="opacity:.8;margin-right:8px;"></i>Damaged Contact Lenses (هالك)</h2>
                <p>View all damaged CL entries and recover them back to stock when repaired or re-evaluated.</p>
            </div>
    ...
    <div class="row" style="margin-bottom:22px;">
        <div class="col-md-4">
            <div class="stat-box red">
                ...
                <div class="lbl"><i class="bi bi-exclamation-triangle"></i> Total Damaged CLs</div>
            </div>
        </div>
        <div class="col-md-4">
    ...
    <div style="background:#fff;border-radius:12px;padding:24px;box-shadow:0 2px 10px rgba(0,0,0,.05);border:1px solid #e8ecf7;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;padding-bottom:12px;border-bottom:2px solid #e8ecf7;">
            <h4 style="margin:0;font-size:15px;font-weight:700;color:#2c3e50;">
                <i class="bi bi-list-ul" style="color:#1565c0;"></i> Damaged CL Entries
            </h4>
            <span style="background:#bbdefb;color:#1565c0;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:700;">
        ...
        @forelse($damaged as $entry)
            ...
            <div class="entry-row {{ $fullyRecovered ? 'fully-recovered' : '' }}" id="entry-{{ $entry->id }}">
                <div class="row" style="align-items:center;">
                    ...
                    <div class="col-md-4">
                        <div class="cl-name">
                            <i class="bi bi-eye" style="color:#1565c0;"></i>
                            {{ optional($entry->product)->description ?? ('Product #' . $entry->product_id) }}
                        </div>
                        <div class="cl-meta">
                            @if($entry->product)
                                ID: <strong>{{ $entry->product->product_id }}</strong>
                                @if($entry->product->brand_id) · {{ $entry->product->brand_id }} @endif
                                @if($entry->product->lense_use) · {{ $entry->product->lense_use }} @endif
                                @if($entry->product->power) · Power: {{ $entry->product->power }} @endif
                            @endif
                        </div>
                        ...
                        <div style="margin-top:6px;display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap;">
                            <span style="background:#eff6ff;border:1px solid #bfdbfe;color:#1d4ed8;
                                ...
                                <i class="bi bi-receipt"></i> Invoice: {{ $invoice->invoice_code }}
                            </span>
                            @if($po)
                            <span style="background:#fef3c7;border:1px solid #fde68a;color:#92400e;
                                         padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;">
                                <i class="bi bi-eye"></i> PO: {{ $po->po_number }}
                            </span>
                            @endif
                        </div>
                        @endif
                ...
                @if(!$fullyRecovered)
                    <div class="recover-form" id="recoverForm-{{ $entry->id }}">
                        <form action="{{ route('dashboard.lens-purchase-orders.cl.recover-damaged', $entry->id) }}"
                              method="POST">
                            @csrf
                            <div class="row" style="align-items:flex-end;">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="notes" class="form-control form-control-sm"
                                           placeholder="e.g. Re-evaluated, repaired, returned to supplier..."
        ...
        @empty
            <div style="text-align:center;padding:50px;color:#aaa;">
                ...
                <h4 style="color:#666;">No damaged CL entries found</h4>
                <p style="font-size:13px;">Damaged contact lenses will appear here when marked as هالك during re-orders.</p>
            </div>
        @endforelse
