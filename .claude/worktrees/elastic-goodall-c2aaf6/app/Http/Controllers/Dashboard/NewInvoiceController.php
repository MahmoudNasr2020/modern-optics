...
class NewInvoiceController extends Controller
{
    ...
    public function create($customer_id)
    {
        ...
        // Get default branch
        $defaultBranch = $user->hasBranch() ? $user->branch : $branches->first();
        ...
        return view('dashboard.pages.invoice-new.create', compact(
            'customer',
            'branches',
            'defaultBranch',
            'categories',
    ...
    public function storeItemInSession(Request $request)
    {
        ...
        try {
            ...
            if (!$item) {
                return response()->json([
                    'success' => false,
                    'message' => 'Item not found or out of stock in this branch'
                ], 404);
            }
    ...
    public function filterLenses(Request $request)
    {
        ...
        // Get lenses that have stock in the specified branch
        $query = glassLense::whereHas('branchStocks', function($q) use ($branch_id) {
            $q->where('branch_id', $branch_id)
                ->where('quantity', '>', 0);
        })
            ->where('branch_id', $branch_id); // Keep this for backward compatibility
        ...
        if ($request->production) {
            $query->where('production', $request->production);
        }
        ...
        // Eager load branch stocks for the specific branch
        $lenses = $query->with(['branchStocks' => function($q) use ($branch_id) {
            $q->where('branch_id', $branch_id);
        }])
        ...
        $formatted = $lenses->map(function($lens) use ($branch_id) {
            // Get stock from BranchStock polymorphic relationship
            $branchStock = $lens->branchStocks->first();
            ...
            return [
                ...
                'brand' => $lens->brand,
                'production' => $lens->production,
                'retail_price' => (float) $lens->retail_price,
                'stock' => $branchStock ? $branchStock->available_quantity : 0, // From BranchStock
                'total_quantity' => $branchStock ? $branchStock->quantity : 0,
    ...
    public function addLensesToInvoice(Request $request)
    {
        ...
        try {
            ...
            if ($skippedCount > 0) {
                $message .= " ({$skippedCount} skipped - already exists or out of stock)";
            }
    ...
    public function validateStock(Request $request)
    {
        ...
        foreach ($draft['items'] as $item) {
            if ($item['type'] === 'product') {
                ...
                $currentStock = $this->getLensWithStock(
                    $item['product_id'],
                    $item['branch_id']
                );
            }
    ...
    {
        // Get the actual lens record
        $lens = glassLense::with(['branchStocks' => function($q) use ($branch_id) {
            $q->where('branch_id', $branch_id);
        }])
            ->where('product_id', $product_id)
            ->first();
        ...
        // Get branch stock using polymorphic relationship
        ...
        return [
            ...
            'stock' => $branchStock->available_quantity, // Use available_quantity
            'branch_id' => $branch_id,
            'branch_name' => $branch->name ?? 'Unknown'
        ];
    }
    ...
    private function validateStockBeforeSave($draft)
    {
        ...
        foreach ($draft['items'] as $item) {
            // Determine stockable type
            $stockableType = $item['type'] === 'lens' ? 'App\\glassLense' : 'App\\Product';

            // For lenses, we need to get the actual lens ID, not product_id
            if ($item['type'] === 'lens') {
                $lens = glassLense::where('product_id', $item['product_id'])
                ...
                if (!$product) {
                    $errors[] = "Product not found for: {$item['description']}";
                    continue;
                }

                $stockableId = $product->id;
            }
    ...
    private function reduceStock($item, $invoiceId = null)
    {
        ...
        // Determine stockable type and get the actual model ID
        if ($item['type'] === 'lens') {
            $lens = glassLense::where('product_id', $item['product_id'])
                ->where('branch_id', $branchId)
                ->first();
            ...
            if (!$lens) {
                throw new \Exception("Lens not found: {$item['description']}");
            }

            $stockableType = 'App\\glassLense';
            $stockableId = $lens->id;

            // Also update the old lens amount field for backward compatibility
            $lens->decrement('amount', $item['quantity']);
        ...
        // Reduce quantity using model method
        ...
        $movement = StockMovement::create([
            'branch_id' => $branchId,
            'product_id' => $stockableType === 'App\\Product' ? $item['product_id'] : null, // For backward compatibility
    ...
    private function reserveStock($item, $branchId, $invoiceId)
    {
        ...
        try {
            if ($item['type'] === 'product') {
                ...
                \Log::info("Stock reserved: Product {$item['product_id']} x{$item['quantity']} in branch {$branchId}");
                ...
                if (!$lens) {
                    throw new \Exception("Lens {$item['product_id']} not found");
                }

                // Reserve stock in branch
                $lens->reserveInBranch($branchId, $item['quantity']);
                ...
                \Log::info("Stock reserved: Lens {$item['product_id']} x{$item['quantity']} in branch {$branchId}");
            }
